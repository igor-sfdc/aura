...
public class BundleFileOrig extends File {
	...
	public BundleFileOrig(String nameOrPath, Class<?> clientClass) {
		this(nameOrPath, BundleUtil.getBundle(clientClass));
	}
	
	private BundleFileOrig(String nameOrPath, Bundle bundle) {
		...
		this.bundle = !super.exists() ? bundle : null;		
	}
	...
	private static String computeParent(String nameOrPath) {
		int index = nameOrPath.lastIndexOf(PATH_SEPARATOR);
		return index != -1 ? nameOrPath.substring(0, index) : null;
	}
	...
	public boolean equals(Object obj) {
		...
		if( obj instanceof BundleFileOrig) {
			return getAbsolutePath().equals(((BundleFileOrig)obj).getAbsolutePath());
		} else {
			return false;
		}
	}
	...
	public File getParentFile() {
		...
		return new BundleFileOrig(getParent(), bundle);
	}
	...
	public String[] list(FilenameFilter filter) {
		...
		while(entries.hasMoreElements()) {
			...
			BundleFileOrig nextFile = new BundleFileOrig(nextEntryElement, bundle);
	...
	public File[] listFiles() {
		...
		Enumeration<String> entries = bundle.getEntryPaths(path);
		Collection<BundleFileOrig> files = Lists.newArrayList();
		while(entries.hasMoreElements()) {
			String nextEntryElement = entries.nextElement();
			BundleFileOrig nextFile = new BundleFileOrig(nextEntryElement, bundle);
			files.add(nextFile);
		}	
		return files.toArray(new BundleFileOrig[0]);
	}
