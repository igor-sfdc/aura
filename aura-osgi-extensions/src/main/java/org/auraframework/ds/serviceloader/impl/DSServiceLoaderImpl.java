...
public class DSServiceLoaderImpl implements ServiceLoader {
	...
	public DSServiceLoaderImpl() {
    	AuraDSLog.get().info("[" + getClass().getSimpleName() + "] " + " Instantiated");
	}

	@Reference (multiple=true, dynamic=true)
	protected void setAuraService(AuraServiceProvider auraDSService) {
		AURA_DS_SERVICE_HANDLER.set(serviceMap, auraDSService, counters);
	}
	
	protected void unsetAuraService(AuraServiceProvider auraDSService) {
		AURA_DS_SERVICE_HANDLER.unset(serviceMap, auraDSService);
	}
	...
	static class ServiceMapHandler {
		...
		private void addImplementation(Map<Class<?>, Set<AuraServiceProvider>> serviceMap, Class<?> implementedInterfaceOrClass, AuraServiceProvider value, Counters counters) {
	    	...
	    	AuraDSLog.get().info("Added implementation of AuraDSService->" + implementedInterfaceOrClass.getSimpleName() + " by " + value + " - " + counters.added + "/" + counters.lookedAt);				
		}
		...
		private void removeImplementation(Map<Class<?>, Set<AuraServiceProvider>> serviceMap, Class<?> implementedInterfaceOrClass, AuraServiceProvider value) {
			...
			if (implementingServiceInstances != null) {
				...
				if (implementingServiceInstances.isEmpty()) {
			    	...
			    	AuraDSLog.get().info("Removed the last implementation of AuraDSService->" + implementedInterfaceOrClass.getSimpleName() + " by " + value + " - " + serviceMap.size());				
				} else {
			    	AuraDSLog.get().info("Removed implementation of AuraDSService->" + implementedInterfaceOrClass.getSimpleName() + " by " + value + " - " + serviceMap.size());				
				}
			}
		}
	}
